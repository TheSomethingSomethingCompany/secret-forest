...
const router = require('../app/routes/requests/AcceptRequest'); // replace with the path to your router
...
server.use('/acceptRequest', router); // attach your router
...
describe('If able to cancel request', () => {
    ...
    it('should return status 201', async () => {
        ...
        db.tx.mockImplementation((callback) => {
            const t = {
                none: jest.fn().mockImplementation((query, values) => {
                    if (query.includes('DELETE FROM request WHERE ("toMemberID" = $1 AND "fromMemberID" = $2)') && values[0] == 'mockLoggedInUserMemberID' && values[1] == 'mockTestMemberID') {
                        return Promise.resolve();
                    } else if (query.includes('INSERT INTO chat("memberID1", "memberID2") VALUES ($1, $2)') && values[0] == 'mockLoggedInUserMemberID' && values[1] == 'mockTestMemberID') {
                        return Promise.resolve();
                    }
                }),
            };
        ...
        const res = await 
        request(server)
        .post('/acceptRequest/api'); // Specify the endpoint. In this current express app, it is the only router we added
