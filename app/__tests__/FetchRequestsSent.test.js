...
const router = require('../app/routes/requests/FetchRequestsSent'); // replace with the path to your router
...
server.use('/fetchRequestsSent', router); // attach your router
...
describe('If fetch is successful', () => {
    it('should return status 200 and the data', async () => {
        ...
        const res = await 
        request(server)
        .get('/fetchRequestsSent/api');

        const resBody = JSON.parse(res.text)
        expect(resBody.status).toBe(200);
        expect(resBody.message).toBe('Retrieved requests sent successfully');
...
describe('If fetch is unsuccessful', () => {
    it('should return status 500 and the error message', async () => {
        ...
        expect(resBody.message).toBe('Failed to retrieve sent requests');
        expect(resBody.error).toBe('mockError');
    });
});
