...
router.post("/api", async (req, res) => {
  ...
  try {
    ...
    console.log("[SUCCESS]: USER FETCHED SUCCESSFUL");
    console.log("[USER]: ", user);
    console.log("[QUERIED USER MEMBERID DOESN'T MATCH LOGGED IN USER MEMBERID] -> UserID: " + queriedUserID + " Session UserID: " + loggedInUserID);
    // END DEBUGGING


    // Check if the logged in user has sent or received a request
    ...
    if(requestData != null) // If the requestData is not null, then a request exists
    {
      ...
      if(requestData.fromMemberID == loggedInUserID) // If the fromMemberID is the logged in user, then the logged in user has sent the request to the queried user
      {
        hasRequest = 1;
      }
