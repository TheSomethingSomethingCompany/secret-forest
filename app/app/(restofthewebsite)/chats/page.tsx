  ...
  useEffect(() => {
    ...
    ws.current.onmessage = (event: any) => {
      ...
      if (response.broadcast) {
        if (response.chatID == chatID.current) getMessages(chatID.current);
      } else {
  ...
  async function getChats() {
    ...
    if (res.data) {
      ...
      for (let i = 0; i < res.data.length; i++) {
        ...
        chatsProfilePicturesContainer.set(res.data[i].chatID, pfpPath.data);
      }
        ...
        {chatsList.length == 0 && (
          <div className="flex flex-row justify-between items-center p-4 border-2 border-transparent rounded-lg h-full max-h-[8rem] m-4 shadow">
            <p className="text-[1rem] font-normal break-words w-[20rem]">
              You don't have any chats yet! Start a new chat today!
            </p>
          </div>
        )}
        {chatsList.length != 0 &&
          chatsList.map((chat) => {
            return (
              <UserConversation
                ...
                chatID={chat.chatID}
                profilePicture={chatsProfilePictures.get(chat.chatID)}
                name={chat.name}
          ...
          {messagesList.map((message) => {
            return (
              <ChatBubble
                ...
                hasAttachment={false}
                attachmentExt={
                  message.fileExtension ? message.fileExtension : null
                }
                isYou={message.isYou}
